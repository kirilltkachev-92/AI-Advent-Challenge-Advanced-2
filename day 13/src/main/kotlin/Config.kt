...
/**
 * Конфигурация шлюза: env имеет приоритет над .env; секреты в код не попадают.
 * Порт по умолчанию 8013 (8000 + номер дня), переопределяется через PORT.
 */
object Config {
    ...
    fun port(): Int = envValue("PORT")?.toIntOrNull() ?: 8013
    fun bindHost(): String = envValue("BIND_HOST") ?: "127.0.0.1"

    /** Rate limit: запросов к /v1/chat с одного IP за минуту. */
    fun rateLimitPerMin(): Int = envValue("RATE_LIMIT_PER_MIN")?.toIntOrNull() ?: 10
