1) Причина: В `HistoryStore` используется значение `capacity`, которое инициализируется значением из `.env`. Если HISTORY_SIZE установлен в 0, это приводит к тому, что `entries.size == capacity` всегда истинно, вызывая удаление элементов без добавления новых. Это происходит на строке 19 файла `HistoryStore.kt`.
...
class HistoryStore(private val capacity: Int = Config.historySize()) {
    ...
    fun add(task: String, phrase: String) {
        if (entries.size >= capacity && capacity > 0) entries.removeFirst()
        entries.addLast(
            HistoryEntry(
