...
```bash
./gradlew test        # 60 тестов (kotlin-test + JUnit 5), сеть не нужна
```

Что покрыто (`src/test/kotlin`):

- `ConfigTest` (14) — чистый парсер `parseDotEnvLine`: пустые строки, комментарии,
  строки без `=`/ключа → null; обрезка пробелов, снятие парных кавычек;
- `MotivatorTest` (12) — доменная логика поверх фейкового `ChatClient`: что уходит
  в LLM (задача как user-сообщение, temperature 1.1, системный промпт), чистка
  ответа (trim, схлопывание whitespace, снятие обрамляющих `"…"` и `«…»`),
  транспортные ошибки → `MotivationResult.Failed` вместо исключения;
- `HistoryStoreTest` (10) — кольцевой буфер: вытеснение старых, порядок «свежие
  первыми», `capacity <= 0` отключает историю, `clear()`, конкурентные `add`;
- `HttpApiTest` (24) — интеграция: живой `HttpServer` на случайном порту (порт 0),
  DeepSeek подменён фейком — сеть не трогается. Ветви обороны `/v1/motivate`
  (405 → 413 → 400 → 502 → 200; кейс «413 до чтения тела» — raw-сокетом
  с поддельным `Content-Length`), `X-Request-Id` во всех ответах, `/healthz`,
  `/v1/history` (включая DELETE), `/v1/limits`, HTML и стабильные id страницы UI.

UI-smoke живёт в дне 3: `day 3/smoke/run_smoke.py` — Playwright-раннер гоняет
реальный Chrome по этому сервису (сервис должен быть поднят через `./run.sh`).
