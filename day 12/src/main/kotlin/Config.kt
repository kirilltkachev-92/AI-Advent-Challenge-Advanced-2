 ...
 * Ключ нужен только командам, которые реально гоняют агента (`run`/`one`);
 * `catalog` и `sanitize` — детерминированные офлайн-эшелоны, они работают
 * без ключа и без сети (зависимость от LLM в SeriesRun ленивая).
 */
object Config {
    ...
    /** Модель агента: одна и та же во всех трёх режимах обороны, иначе сравнение нечестное. */
    fun agentModel(): String = envValue("AGENT_MODEL") ?: "deepseek-v4-flash"
}
