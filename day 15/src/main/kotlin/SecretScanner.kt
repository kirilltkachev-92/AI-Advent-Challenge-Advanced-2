...
enum class SecretType(val placeholder: String) {
    ...
    // Охраняемое значение без известного префикса (DB_PASSWORD, флаг/деплой-токен):
    // ловится точным сравнением, а не regex. Hardening дня 15.
    GUARDED("[REDACTED_GUARDED]"),
}
...
/**
 * Детектор секретов, общий для input- и output-guard. Проходы:
 * 0) охраняемые значения (ProtectedValues) точным сравнением — секреты без regex-
 *    префикса (DB_PASSWORD, флаг/деплой-токен); высший приоритет [hardening дня 15];
 ...
 * 4) охраняемые значения в кодировках (base64/hex/reverse/ROT13) — закодированная утечка.
...
class SecretScanner {
    ...
    /** Полный скан текста: все проходы, находки отсортированы по позиции. */
    fun scan(text: String): List<Finding> {
        ...
        // Проход 4 (hardening): охраняемые значения в кодировках (base64/hex/reverse/ROT13),
        // которые не разворачиваются под известный regex — чтобы секрет не утёк закодированным.
        guardedTransformHits(text).forEach { addIfNoOverlap(hits, it) }
    ...
    private fun directHits(text: String, via: String): List<RawHit> {
        val hits = mutableListOf<RawHit>()
        // Проход 0 (высший приоритет, hardening дня 15): охраняемые значения точным
        // сравнением — DB_PASSWORD, флаг/деплой-токен и пр. не имеют regex-префикса и
        // раньше уходили в модель немаскированными. Не полагаемся на модель — режем у входа.
        ProtectedValues.all().forEach { secret ->
            var i = text.indexOf(secret)
            while (i >= 0) {
                hits += RawHit(SecretType.GUARDED, i, i + secret.length, secret, via)
                i = text.indexOf(secret, i + secret.length)
            }
        }
    ...
    // ── Проход 4: охраняемые значения в кодировках ───────────────────────

    /** Ищет base64/hex/reverse/ROT13-формы охраняемых значений — закодированную утечку. */
    private fun guardedTransformHits(text: String): List<RawHit> {
        val hits = mutableListOf<RawHit>()
        ProtectedValues.all().forEach { secret ->
            encodedForms(secret).forEach { form ->
                if (form.length < 8) return@forEach
                var i = text.indexOf(form)
                while (i >= 0) {
                    hits += RawHit(SecretType.GUARDED, i, i + form.length, form, "encoded")
                    i = text.indexOf(form, i + form.length)
                }
            }
        }
        return hits
    ...
    private fun encodedForms(s: String): List<String> = listOf(
