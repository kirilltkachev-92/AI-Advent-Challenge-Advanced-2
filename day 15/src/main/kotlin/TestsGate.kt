...
class TestsGate(
    ...
    private val compileEnabled: Boolean = Config.compileGate(),
) {
    ...
    fun check(task: ProvocationTask, code: String): GateResult {
        ...
        // На слабом боевом сервере компиляцию можно выключить (COMPILE_GATE=false):
        // остаются acceptance-проверки, а стражи/ревьюер работают в полную силу.
        if (!compileEnabled) return GateResult.Passed
        return compile()
    }
